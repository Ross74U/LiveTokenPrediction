...
from fileAsStream import AudioFileStream   
...
AUDIO_FILE: str = "Audio/joe_rogan1.wav"
...
class VoskWhisper:
    ...
    def _audio_data_worker(self, target_sample_rate=16000, buffer_size=512):
        ...
        file_stream : AudioFileStream = AudioFileStream(filename=AUDIO_FILE, chunk_size=1024, target_sample_rate=16000)
        file_stream.start_stream()
        
        print("audio filestream initialized")
        buffer = bytearray()
        silero_buffer_size = 2 * buffer_size  # silero complains if too short, buffer size is 2x because int16 to bytes


        # chunk duration which only depends on the FileStream Parameters
        bytes_per_sample = pyaudio.get_sample_size(file_stream.format)
        chunk_duration = file_stream.chunk_size / (file_stream.target_sample_rate * file_stream.channels * bytes_per_sample)

        while file_stream.is_playing:
            data = file_stream.stream.read(file_stream.chunk_size)

            # in order to simulate realtime audio feed. We wait here the
            # exact amount of the duration of the audio chunk
            # time.sleep(chunk_duration)
            
            # Processing raw data
            # putting into silero buffer
            processed_data = preprocess_audio(data, file_stream.target_sample_rate, target_sample_rate)
